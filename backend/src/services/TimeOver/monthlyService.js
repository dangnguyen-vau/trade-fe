...
// Lấy dữ liệu ngày
async function getMonthlyData(params) {
    ...
    try {
        ...
        const result = await axios({
            method: 'GET',
            url: 'http://103.216.117.117:82/api/v1/monthly?timescale=20',
            headers: {
        ...
        const filePath = path.join(dirPath, 'monthly.json');
        ...
        console.error('Lỗi khi lấy dữ liệu monthly', error.message);
        throw error;
    }
}
...
function getLocalMonthlyData() {
    try {
        const filePath = path.join(__dirname, '../..', 'DataBot', 'TimeOver', 'monthly.json');
        ...
        console.error('Lỗi khi đọc file monthly.json:', error.message);
        return null;
    }
}
